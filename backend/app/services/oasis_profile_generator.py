...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        """
        if not self.entity_reader or not self.graph_id:
        ...
        try:
            # 获取实体的边（事实/关系）
            edges = self.entity_reader.get_node_edges(entity.uuid)
            all_facts = set()
            for edge in edges:
                fact = edge.get("fact", "")
                if fact:
            ...
            # 获取所有节点的摘要
            all_nodes = self.entity_reader.get_all_nodes(self.graph_id)
            all_summaries = set()
            for node in all_nodes:
                node_data = node if isinstance(node, dict) else node
                name = node_data.get("name", "") if isinstance(node_data, dict) else getattr(node_data, 'name', '')
                summary = node_data.get("summary", "") if isinstance(node_data, dict) else getattr(node_data, 'summary', '')
                if summary and name != entity.name:
                    all_summaries.add(f"相关实体: {name} - {summary}")
            results["node_summaries"] = list(all_summaries)[:10]
            ...
            logger.info(f"Neo4j检索完成: {entity.name}, 获取 {len(results['facts'])} 条事实, {len(results['node_summaries'])} 个相关节点")
            
        except Exception as e:
            logger.warning(f"Neo4j检索失败 ({entity.name}): {e}")
